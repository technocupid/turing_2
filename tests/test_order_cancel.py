...
def test_cancel_paid_order_with_refund(client: TestClient):
    ...
    payres = rp.json()
    assert payres.get("ok") is True or payres.get("transaction")
    # ensure order marked paid in DB
    got = client.get(f"/api/orders/{order_id}", headers=headers)
    assert got.status_code == 200
    row = got.json()
    assert row.get("status") == "paid" or row.get("last_payment_success") in (True, "True", "1")
    ...
    assert updated.get("last_refund_success") in (True, "True", "1")
